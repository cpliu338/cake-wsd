...
protected function get_file_name($file_path, $name, $size, $type, $error,
    ...
    $name = $this->trim_file_name($file_path, $name, $size, $type, $error,
        $index, $content_range);
    $uniq_name = $this->get_unique_filename(
        $file_path,
    ...
    //Adapted to use user specified name
    return $this->get_user_specified_name($uniq_name);
}
/**
 * Adapted to use user specified name
 */
protected function get_user_specified_name($file_name) {
    // use user specified name no matter what
    if (preg_match('/\.([^.]+)$/', $file_name, $matches)) {
        $file_name = $this->options['user_specified_name'] . '.' . $matches[1];
    }
    return $file_name;
}
